...
const sideBarTool = {
	...
	genSideBar: (RootPath, unDirIncludes, SuffixIncludes) => {
		...
		allDirs.forEach((item) => {
			let dirFiles = filehelper.getAllFiles(
				item,
				unDirIncludes,
				SuffixIncludes
			)
	...
	genSideBarGroup: (
		...
		let allDirs = filehelper.getAllDirs(RootPath, unDirIncludes)
		allDirs.forEach((item) => {
			let children = filehelper.getAllFiles(
				item,
				unDirIncludes,
				SuffixIncludes
			)
			let dirname = item.replace(RootPath, '')
			let titleTemp = dirname.replace(/\\/g, '')
			console.log(titleTemp, children, 'titleTemp')
      children.shift()
			title = titleTemp
			if (children.length > 1) {
				children = children.flatMap(
					(vo, idx) => [[dirname.replace(/\\/g, '/') + vo, vo]]
				)
			}
			let Obj = {
				title,
